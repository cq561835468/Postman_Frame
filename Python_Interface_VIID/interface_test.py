...
from Public.AscII_Decode import Req_Resp,Html_RR
...
class Auto_intest(threading.Thread):
    ...
    def run_test(self,device,Module,request_name,test_list):
        ...
        if len_it == '0':
            va = "newman run "+test_list[1] +" -d "+ test_list[0] +" -g "+self.path_base+self.device+"/"+self.device+".postman_environment.json"+" -r html,json --reporter-html-export "+self.report_path+".html" + " --reporter-json-export " + self.report_path + ".json"
        else:
        ...
        #print self.device, self.Module, self.report_path+".json", self.report_path+".html", self.now_time, self.jk_timei
        if self.Module != '__init':
            self.CMO_Post(self.device, self.Module, self.report_path+".json", self.report_path+".html", self.now_time, self.jk_time)
            self.Html_Change()

    def Html_Change(self):
        RR = Req_Resp(self.report_path+r".json")
        Return_Json = RR.run()
        HR = Html_RR(Return_Json, self.report_path+r".html")
        HR.run()
...
def Retrans(State, CJR):
    '''重传请求'''
    Retrans = Get_Conf("CMO_Conf", "Retrans")
    if Retrans == '1':
        if State == "Post":
            if CJR["State"] == "NoOpen" or not CJR["State"]:  # NoOpen|False
            ...
            #print CJR["State"]
            if CJR["State"] == "NoOpen" and not CJR["State"]:
                logs.debug(CJR["String"])
            else:
                #print "remove"
                os.remove(path+".json")
...
def Get_Conf(session,value):
    cf = ConfigParser.ConfigParser()
    cf.read(os.getcwd()+"/Conf/Conf.conf")
    re = cf.get(session, value)
    return re

def run_list_thread(device,module,fold_name,path_base,jk_time):
    t = Auto_intest(device,module,fold_name,path_base,jk_time)
    num = Get_Conf("Thread", "Thread_Num")
    while len(threading.enumerate()) > int(num):
          time.sleep(5)
          logs.debug("%s 线程请稍等，线程数为%s" % (module,num))
    t.start()
    logs.debug("[%s]产品 [%s]模块 线程开始" % (device,module))
    ...
    if len_module == 1:
        if sys.argv[0] != "CmoReload":# CMO失败的数据重传
            ...
            for x in Cache_path:
                ...
                path = Cacheos + os.path.splitext(x)[0] # 文件路径
                path_spar = x.split("#") #分割项目和模块
                #print path_spar[0],path_spar[1],path+".json",path+".html", path_spar[3][:-5], now_time
                CJ = Create_Json(path_spar[0],path_spar[1],path+".json",path+".html", path_spar[3][:-5], path_spar[3][:-5])
                CJR = CJ.Run()
                Retrans("Retrans", CJR)

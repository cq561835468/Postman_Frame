...
class Create_Json():
    ...
    def Run(self):
        if self.Get_CF("CMO_Conf","Open_ID") == "0": #
            return {"State":'NoOpen',"String":"NoOpen"}
    ...
    def Public_Table(self):
        ...
        Re_Data['Total_run_duration'] = self.Time_From(str(round(self.taglist2[1].text).split(' '))) # 总运行时间
        Re_Data['Total_data_received'] = str(round(self.Json['run']['transfers']['responseTotal'])) +'B' # 传输数据大小
        Re_Data['Average_response_time'] = str(round(self.Json['run']['timings']['responseAverage'])) +"ms" # 平均响应时间

...
class Auto_intest(threading.Thread):
    def __init__(self,device,module,fold_name,path_base):
    ...
    def run_test(self,device,Module,request_name,test_list):
        ...
        self.CMO_Post(self.device, self.Module, self.report_path+".json", self.report_path+".html", self.now_time)

    def CMO_Post(self, device, Module, report_path_j, report_path_h, time):
        '''发送请求CMO'''
        #print("发送CMO请求")
        #print(report_path_j)
        #print(report_path_h)
        #try:
        CJ = Create_Json(device, Module, report_path_j, report_path_h, time)
        CJR = CJ.Run()
        if CJR["State"]:
...
def run_list_thread(device,module,fold_name,path_base):
    t = Auto_intest(device,module,fold_name,path_base)
    t.start()
    ...
    if len_module == 1:
        ...
        if sys.argv[2].lower() == 'all': #所有模块用例执行
            ...
            for x in Module_Dir:
                ...
                run_list_thread(device,x,fold_name,path_base)
            ...
            run_list_thread(device,module,fold_name,path_base)
        #time.sleep(300)
        ...
        for i in sys.argv[2:]:
            ...
            run_list_thread(device,i,fold_name,path_base)
        #time.sleep(300)
